...
const PersonalDetails = ({ onNext, onBack, isReadOnly }) => {
  ...
  const ic = (f) => `w-full px-3 py-2.5 rounded-lg border ${errors[f] ? 'border-red-400 bg-red-50/30' : 'border-gray-300'} focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all ${isReadOnly ? 'bg-gray-100 cursor-not-allowed text-gray-500' : ''}`;
  const sc = (f) => `${ic(f)} ${isReadOnly ? '' : 'bg-white'}`;
  ...
  return (
    <SectionLayout title="Personal Details" subtitle="Please provide your complete personal, contact, and academic information." onNext={handleNext} onBack={onBack} hideNext={isReadOnly}>
      <div className="space-y-8">
        ...
        <fieldset className="border border-gray-200 rounded-xl p-5 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-1">
              {label('Category', true)}
              <select value={d.category} onChange={e => set('category', e.target.value)} className={sc('category')} disabled={isReadOnly}>
            ...
            <div className="space-y-1 flex items-end gap-3 pb-1">
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" checked={d.disability} onChange={e => set('disability', e.target.checked)} className="w-4 h-4 rounded" disabled={isReadOnly} />
        ...
        <fieldset className="border border-gray-200 rounded-xl p-5 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="space-y-1">
              ...
              <input value={d.name} onChange={e => set('name', e.target.value)} className={ic('name')} placeholder="Dr. Full Name" disabled={isReadOnly} />
              {err('name')}
            </div>
            <div className="space-y-1">
              {label("Father's Name", true)}
              <input value={d.fatherName} onChange={e => set('fatherName', e.target.value)} className={ic('fatherName')} placeholder="Father's full name" disabled={isReadOnly} />
              {err('fatherName')}
            </div>
            <div className="space-y-1">
              {label('Nationality', true)}
              <input value={d.nationality} onChange={e => set('nationality', e.target.value)} className={ic('nationality')} disabled={isReadOnly} />
              {err('nationality')}
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="space-y-1">
              ...
              <input type="date" value={d.dob} max={new Date().toISOString().split("T")[0]} onChange={e => set('dob', e.target.value)} className={ic('dob')} disabled={isReadOnly} />
              {err('dob')}
            </div>
            <div className="space-y-1">
              {label('Gender', true)}
              <select value={d.gender} onChange={e => set('gender', e.target.value)} className={sc('gender')} disabled={isReadOnly}>
                <option value="">Select</option>
            ...
            <div className="space-y-1">
              ...
              <select value={d.maritalStatus} onChange={e => set('maritalStatus', e.target.value)} className={sc('maritalStatus')} disabled={isReadOnly}>
                <option value="">Select</option>
            ...
            <div className="space-y-1">
              ...
              <input value={d.aadhar} onChange={e => set('aadhar', e.target.value.replace(/\D/g, '').slice(0, 12))} className={ic('aadhar')} placeholder="12 digits" maxLength={12} disabled={isReadOnly} />
              {err('aadhar')}
            </div>
          </div>
        </fieldset>
        ...
        <fieldset className="border border-gray-200 rounded-xl p-5 space-y-4">
          ...
          <div className="space-y-1">
            {label('Address', true)}
            <textarea value={d.corrAddress} onChange={e => set('corrAddress', e.target.value)} className={`${ic('corrAddress')} h-20`} placeholder="Street, Locality..." disabled={isReadOnly} />
            {err('corrAddress')}
          </div>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="space-y-1">
              {label('City', true)}
              <input value={d.corrCity} onChange={e => set('corrCity', e.target.value)} className={ic('corrCity')} disabled={isReadOnly} />
              {err('corrCity')}
            </div>
            <div className="space-y-1">
              {label('District', true)}
              <input value={d.corrDistrict} onChange={e => set('corrDistrict', e.target.value)} className={ic('corrDistrict')} disabled={isReadOnly} />
              {err('corrDistrict')}
            </div>
            <div className="space-y-1">
              {label('State', true)}
              <select value={d.corrState} onChange={e => set('corrState', e.target.value)} className={sc('corrState')} disabled={isReadOnly}>
            ...
            <div className="space-y-1">
              {label('Pincode', true)}
              <input value={d.corrPincode} onChange={e => set('corrPincode', e.target.value.replace(/\D/g, '').slice(0, 6))} className={ic('corrPincode')} maxLength={6} placeholder="6 digits" disabled={isReadOnly} />
              {err('corrPincode')}
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-1">
              ...
              <input value={d.phone} onChange={e => set('phone', e.target.value)} className={ic('phone')} placeholder="Optional" disabled={isReadOnly} />
            </div>
          </div>
        </fieldset>
        ...
        <fieldset className="border border-gray-200 rounded-xl p-5 space-y-4">
          ...
          <label className="flex items-center gap-2 cursor-pointer mb-2">
            <input type="checkbox" checked={d.sameAsCorrespondence} onChange={copyAddress} className="w-4 h-4 rounded" disabled={isReadOnly} />
            <span className="text-sm text-gray-600">Same as Correspondence Address</span>
          </label>
          <div className="space-y-1">
            {label('Address', true)}
            <textarea value={d.permAddress} onChange={e => set('permAddress', e.target.value)} className={`${ic('permAddress')} h-20 ${d.sameAsCorrespondence ? 'bg-gray-100/70 text-gray-500 cursor-not-allowed opacity-80' : 'bg-white'}`} readOnly={d.sameAsCorrespondence || isReadOnly} disabled={isReadOnly} />
            {err('permAddress')}
          </div>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div className="space-y-1">
              {label('City', true)}
              <input value={d.permCity} onChange={e => set('permCity', e.target.value)} className={`${ic('permCity')} ${d.sameAsCorrespondence ? 'bg-gray-100/70 text-gray-500 cursor-not-allowed opacity-80' : 'bg-white'}`} readOnly={d.sameAsCorrespondence || isReadOnly} disabled={isReadOnly} />
              {err('permCity')}
            </div>
            <div className="space-y-1">
              {label('District', true)}
              <input value={d.permDistrict} onChange={e => set('permDistrict', e.target.value)} className={`${ic('permDistrict')} ${d.sameAsCorrespondence ? 'bg-gray-100/70 text-gray-500 cursor-not-allowed opacity-80' : 'bg-white'}`} readOnly={d.sameAsCorrespondence || isReadOnly} disabled={isReadOnly} />
              {err('permDistrict')}
            </div>
            <div className="space-y-1">
              {label('State', true)}
              <select 
                ...
                disabled={d.sameAsCorrespondence || isReadOnly}
              >
                <option value="">Select State</option>
                {INDIAN_STATES.map(s => <option key={s} value={s}>{s}</option>)}
              </select>
            ...
            <div className="space-y-1">
              {label('Pincode', true)}
              <input value={d.permPincode} onChange={e => set('permPincode', e.target.value.replace(/\D/g, '').slice(0, 6))} className={`${ic('permPincode')} ${d.sameAsCorrespondence ? 'bg-gray-100/70 text-gray-500 cursor-not-allowed opacity-80' : 'bg-white'}`} maxLength={6} readOnly={d.sameAsCorrespondence || isReadOnly} disabled={isReadOnly} />
              {err('permPincode')}
            </div>
          </div>
          <div className="space-y-1 md:w-1/2">
            {label('Phone (Permanent)', false)}
            <input value={d.permPhone} onChange={e => set('permPhone', e.target.value)} className={`${ic('permPhone')} ${d.sameAsCorrespondence ? 'bg-gray-100/70 text-gray-500 cursor-not-allowed opacity-80' : 'bg-white'}`} readOnly={d.sameAsCorrespondence || isReadOnly} disabled={isReadOnly} />
          </div>
        </fieldset>
        ...
        <fieldset className="border border-gray-200 rounded-xl p-5 space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-1">
              ...
              {(d.specialization || ['']).map((s, i) => (
                <div key={i} className="flex gap-2">
                  <input value={s} onChange={e => handleSpecialization(i, e.target.value)} className={ic('specialization')} placeholder={`Specialization ${i + 1}`} disabled={isReadOnly} />
                  {i === 0 && (d.specialization || ['']).length < 2 && !isReadOnly && (
            ...
            <div className="space-y-1">
              ...
              <input value={d.scopusId} onChange={e => set('scopusId', e.target.value)} className={ic('scopusId')} placeholder="Optional" disabled={isReadOnly} />
            </div>
          </div>
          <div className="space-y-1">
            {label('PhD Thesis Title', true)}
            <input value={d.phdTitle} onChange={e => set('phdTitle', e.target.value)} className={ic('phdTitle')} placeholder="Full title of PhD thesis" disabled={isReadOnly} />
            {err('phdTitle')}
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-1">
              {label('PhD University / Institution', true)}
              <input value={d.phdUniversity} onChange={e => set('phdUniversity', e.target.value)} className={ic('phdUniversity')} disabled={isReadOnly} />
              {err('phdUniversity')}
            </div>
            <div className="space-y-1">
              {label('PhD Award Date', true)}
              <input type="date" value={d.phdDate} max={new Date().toISOString().split("T")[0]} onChange={e => set('phdDate', e.target.value)} className={ic('phdDate')} disabled={isReadOnly} />
              {err('phdDate')}
            </div>
          </div>
        ...
        <fieldset className="border border-gray-100 rounded-xl p-5 space-y-4 bg-gray-50/50">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-1">
              ...
              <input type="date" value={d.lastPromotionDate} max={new Date().toISOString().split("T")[0]} onChange={e => set('lastPromotionDate', e.target.value)} className={ic('lastPromotionDate')} disabled={isReadOnly} />
            </div>
            <div className="space-y-1">
              {label('Designation', false)}
              <input value={d.lastPromotionDesignation} onChange={e => set('lastPromotionDesignation', e.target.value)} className={ic('lastPromotionDesignation')} disabled={isReadOnly} />
            </div>
            <div className="space-y-1">
              {label('Pay Scale', false)}
              <input value={d.lastPromotionPayScale} onChange={e => set('lastPromotionPayScale', e.target.value)} className={ic('lastPromotionPayScale')} disabled={isReadOnly} />
            </div>
            <div className="space-y-1">
              {label('Department', false)}
              <input value={d.lastPromotionDepartment} onChange={e => set('lastPromotionDepartment', e.target.value)} className={ic('lastPromotionDepartment')} disabled={isReadOnly} />
            </div>
          </div>
        </fieldset>
